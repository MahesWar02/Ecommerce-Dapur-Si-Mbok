...
const data = JSON.parse(fs.readFileSync("./dapur-si-mbok_users.json", "utf8"));
...
async function run() {
  try {
    ...
    const cleaned = data.map((p) => ({
      ...
      resetTokenExpiry: p.resetTokenExpiry
        ? new Date(p.resetTokenExpiry.$date)
        : null,
    }));

    const result = await collection.insertMany(cleaned);
    console.log(`✅ ${result.insertedCount} users berhasil diimport!`);
  } finally {
    await client.close();
  }
}
